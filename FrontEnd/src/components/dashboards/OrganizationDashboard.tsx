...
const SideNavBar = ({ activeTab, onTabChange, profile, onLogout, isSidebarOpen, avatarUrl }: SideNavBarProps) => {
  ...
  return (
    <aside className={cn("h-screen w-64 fixed left-0 top-0 flex flex-col z-40 bg-[#430909] border-r border-[#FCF8F8]/10 shadow-2xl transition-transform duration-300", isSidebarOpen ? "translate-x-0" : "-translate-x-full")}>
      ...
      <div className="px-5 pt-7 pb-5 border-b border-[#FCF8F8]/10">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="text-[#FCF8F8] font-bold text-sm leading-tight tracking-wide">Organization</h1>
            <p className="text-[#FCF8F8]/50 text-[10px] font-mono uppercase tracking-widest mt-0.5">Management Portal</p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-[#FCF8F8]/8 border border-[#FCF8F8]/10">
            {avatarUrl ? (
                ...
                <div className="w-8 h-8 rounded-full bg-[#FCF8F8]/20 border border-[#FCF8F8]/30 flex items-center justify-center flex-shrink-0">
                  <span className="text-[#FCF8F8] text-[11px] font-bold">{initials}</span>
                </div>
            )}
            <div className="min-w-0">
              <p className="text-[#FCF8F8] text-xs font-semibold truncate">{profile.name}</p>
              <p className="text-[#FCF8F8]/40 text-[10px] font-mono truncate">{profile.email}</p>
            </div>
          </div>
        )}
      </div>
